...
class LicenseBoxAPI{
	...
	public function __construct(){ 
		$this->product_id = '2006AB23';
		$this->api_url = 'https://verify.analogit.com/';
	...
	public function verify_license($time_based_check = false, $license = false, $client = false){
		...
		if($time_based_check && $this->verification_period > 0){
			...
			$today = date('d-m-Y');
			if(empty($_SESSION["4e0faf9bc825603"])){
				$_SESSION["4e0faf9bc825603"] = '00-00-0000';
			}
			if($type == 1){
				$type_text = '1 day';
			}elseif($type == 3){
			...
			if(strtotime($today) >= strtotime($_SESSION["4e0faf9bc825603"])){
				$get_data = $this->call_api(
					'POST',
